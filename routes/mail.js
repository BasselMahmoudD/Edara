...
router.post(
  "/send",
  authorized,
  body("email").isString().withMessage("please enter a valid email name"),
  ...
  async (req, res) => {
    try {
      ...
      const productCheck = await query(
        "select * from product where id = ? and warehouse_id = ?",
        [req.body.product_id, req.body.warehouse_id]
      );
      if (
        data.mail != sendmail.email ||
        data.warehouse != sendmail.warehouse_id ||
        (sendmail.product_id != productCheck[0].id &&
          sendmail.warehouse_id != data.warehouse)
      ) {
        res.status(404).json({
          errors: [
            {
...
router.get("/recive", admin, async (req, res) => {
  const query = util.promisify(conn.query).bind(conn);
...
router.get("/recivesuper", authorized, async (req, res) => {
  ...
  const requests = await query(
    "select * from report where email = ?",
    data.mail
  );
...
router.put("/:id", admin, async (req, res) => {
  try {
    const query = util.promisify(conn.query).bind(conn);
    const mail = await query("select * from send_mail where id = ?", [
      req.params.id,
    ]);
    if (!mail[0]) {
    ...
    const email = await query("select * from send_mail where id = ?", [
      req.params.id,
    ]);
    await query("insert into report set ?", email);
    const OrderQuantity = await query("select * from send_mail where id = ?", [
      req.params.id,
    ]);
    const TableProduct = await query(
      "select * from product where id=? and warehouse_id = ?",
      [mail[0].product_id, mail[0].warehouse_id]
    );
    let sum;
    ...
    else if(OrderQuantity[0].response == "Declined") {
      sum = TableProduct[0].quantity;
    }
