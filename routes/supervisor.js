...
router.get("/homeuser",admin,async (req,res)=>{ 
    
        
        const query = util.promisify(conn.query).bind(conn);
        const recive = await query("select * from users where type !=0" )
            res.status(200).json(recive)
   
})
router.get("/homeproduct",admin,async (req,res)=>{ 
    
        
    const query = util.promisify(conn.query).bind(conn);
    const recive = await query("select * from product " )
    recive.map(prod =>{
        prod.image = "http://" + req.hostname + ":4000/" + prod.image
    })
        res.status(200).json(recive)
